...
export default function Pong() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (aiCenter < state.ball.y - 10) {
        state.aiY = Math.min(CANVAS_HEIGHT - PADDLE_HEIGHT, state.aiY + PADDLE_SPEED * 0.45);
      } else if (aiCenter > state.ball.y + 10) {
        state.aiY = Math.max(0, state.aiY - PADDLE_SPEED * 0.45);
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-white rounded-2xl shadow-sm border border-slate-100 max-w-3xl mx-auto">
      <div className="flex justify-between items-center w-full mb-6 px-8 max-w-[600px]">
        ...
        <HowToPlay 
          rules={[
            "Defeat the computer by being the first to score 5 points.",
            "A point is scored when a player fails to return the ball.",
            "The ball speeds up slightly after each paddle hit."
          ]}
          controls={[
            "Up/Down Arrows or W/S to move your paddle up and down.",
